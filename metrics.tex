...
\begin{itemize}
  ...
  \item \textbf{Postman}, utilizat pentru a trimite \^{i}n mod controlat cereri c\u{a}tre endpoint-urile relevante (\texttt{/api/auth/login}, \texttt{/api/profile/by-email/\{email\}}, \texttt{/api/recommendations}) \c{s}i pentru a nota timpii de r\u{a}spuns raporta\c{t}i de instrument. Aceast\u{a} abordare a permis izolarea timpului de procesare al serverului de eventualele \^{i}nt\^{a}rzieri introduse de interfa\c{t}a React.
\end{itemize}
...
\begin{table}[h!]
  ...
  \begin{tabular}{@{}lcccc@{}}
    ...
    \midrule
    Login (\texttt{/api/auth/login})                              & 6 & 300 & 360 & 420 \\
    Profil utilizator (\texttt{/api/profile/by-email/\{email\}})  & 6 & 80  & 90  & 100 \\
...
\end{equation}
\textbf{Interpretare:} C\^{a}t de ``curat\u{a}'' este lista de recomandări? Valoare mare $\rightarrow$ pu\c{t}ine recomandări inutile. \textbf{C\^{a}nd e important\u{a}:} Interfe\c{t}e cu list\u{a} scurt\u{a} (Top-5, Top-10), caz \^{i}n care utilizatorul vede doar primele pozi\c{t}ii.
...
\end{equation}
\textbf{Interpretare:} C\^{a}t de bine recupereaz\u{a} sistemul tot ce era relevant? Valoare mare $\rightarrow$ sistemul nu rateaz\u{a} oportunit\u{a}\c{t}i.
...
\end{equation}
\textbf{Interpretare:} A reu\c{s}it sistemul s\u{a} ``nimereasc\u{a}'' m\u{a}car o recomandare bun\u{a}? \textbf{Particularitate:} Nu \c{t}ine cont de num\u{a}r sau pozi\c{t}ie.

\paragraph{F1@K.}
\textbf{Defini\c{t}ie:} Media armonic\u{a} între Precision@K \c{s}i Recall@K.
\begin{equation}
  F1\text{@}K = 2 \cdot \frac{\text{Precision@}K \cdot \text{Recall@}K}{\text{Precision@}K + \text{Recall@}K}
\end{equation}
\textbf{Interpretare:} M\u{a}soar\u{a} echilibrul între precizie \c{s}i acoperire.

\subsection*{Metrici de evaluare global\u{a}}

\paragraph{Coverage.}
\textbf{Defini\c{t}ie (la nivel de catalog):} Propor\c{t}ia de itemi din catalog care sunt recomanda\c{t}i cel pu\c{t}in o dat\u{a}.
\begin{equation}
  \text{Coverage} = \frac{\text{itemi recomanda\c{t}i (distinc\c{t}i)}}{\text{itemi total \^{i}n catalog}}
\end{equation}
\textbf{Interpretare:} Sistemul recomand\u{a} divers sau doar aceia\c{s}i itemi populari? Valoare mic\u{a} $\rightarrow$ sistem orientat spre popularitate.

\medskip

\noindent\textbf{Coverage-ul nutri\c{t}ional afi\c{s}at \^{i}n interfa\c{t}\u{a}.}
\^{I}n cardurile de recomand\u{a}ri, pentru fiecare aliment este afi\c{s}at un procent de acoperire care indic\u{a} c\u{a}t din deficitul nutri\c{t}ional estimat poate fi acoperit de o por\c{t}ie standard. Formula utilizat\u{a} \^{i}n backend este:
\begin{equation}
  \text{coverage\_pct}(n) = \frac{\text{aport\_por\c{t}ie}(n)}{\text{aport\_por\c{t}ie}(n) + \max(\text{deficit}(n), 0{,}25 \cdot \text{RDI}(n))} \times 100.
\end{equation}
Procentul afi\c{s}at reprezint\u{a} media ponderat\u{a} pe nutrien\c{t}ii acoperi\c{t}i.

\subsection*{Definirea itemilor relevan\c{t}i \^{i}n \textit{VitaBalance}}

Pentru calculul Precision@K, Recall@K \c{s}i Hit Rate, itemii relevan\c{t}i sunt defini\c{t}i pe baza regulilor active, a compatibilit\u{a}\c{t}ii cu profilul (diet\u{a}, alergii, condi\c{t}ii) \c{s}i, unde exist\u{a}, a feedback-ului utilizatorilor (\texttt{rating}, \texttt{tried}, \texttt{worked}).

\subsection*{Rezultate pentru aplica\c{t}ia \textit{VitaBalance}}

\subsubsection*{Metrici algoritmice (offline)}

Tabelul~\ref{tab:rec-metrics} sintetizeaz\u{a} valorile ob\c{t}inute pentru Precision@10, Recall@10, Hit Rate@10, F1@10 \c{s}i Coverage \^{i}n evaluarea offline intern\u{a}.
...
\textbf{Interpretare:} Precision@10 de 0{,}72 indic\u{a} o list\u{a} relativ curat\u{a}, iar Hit Rate@10 de 0{,}95 arat\u{a} c\u{a} majoritatea utilizatorilor primesc cel pu\c{t}in o recomandare relevant\u{a}. Coverage de 0{,}38 sugereaz\u{a} c\u{a} sistemul utilizeaz\u{a} un subset semnificativ, dar nu complet, din catalog.

\subsubsection*{Metrici bazate pe chestionar}

Pentru partea de percep\c{t}ie a utilizatorilor s-a folosit un chestionar Likert (1--5), cu 4 responden\c{t}i.

\begin{table}[h!]
  \centering
  \caption{Scoruri din chestionarul de evaluare (scara 1--5, n=4)}
  ...
  \begin{tabular}{@{}lcc@{}}
    ...
    \midrule
    Relevance Score      & 4{,}50 & 0{,}29 \\
    Explainability Score & 4{,}75 & 0{,}00 \\
    Trust Score          & 4{,}75 & 0{,}29 \\
    Usefulness Score     & 5{,}00 & 0{,}00 \\
    Satisfaction Score   & -- & -- \\
    \bottomrule
  \end{tabular}
\end{table}

Valorile ridicate pentru Explainability, Trust \c{s}i Usefulness confirm\u{a} c\u{a} explica\c{t}iile sunt percepute drept clare \c{s}i utile, iar recomand\u{a}rile influen\c{t}eaz\u{a} pozitiv decizia utilizatorilor. Pentru Satisfaction Score nu a existat distribu\c{t}ia numeric\u{a} complet\u{a} \^{i}n setul de capturi furnizat.

\subsubsection*{Feedback calitativ (întrebări deschise)}

\textbf{Aspecte apreciate:}
\begin{itemize}
  \item design-ul \c{s}i simplitatea interfe\c{t}ei;
  \item func\c{t}ionalitatea de \^{i}nc\u{a}rcare a datelor medicale din PDF;
  \item utilitatea recomand\u{a}rilor \c{s}i a explica\c{t}iilor.
\end{itemize}

\textbf{Îmbunătă\c{t}iri sugerate:}
\begin{itemize}
  \item meniuri mai vizibile;
  \item op\c{t}iune explicit\u{a} pentru obiectiv caloric (deficit / men\c{t}inere / surplus);
  \item op\c{t}iune EN/RO pentru utilizatori non-români.
\end{itemize}
\end{document}